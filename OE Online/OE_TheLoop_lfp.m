...
% 	stimforms=vertcat(stimforms, temp');
	...
	if ~isequal(size(find(cellfun('isempty', waveforms)>0),1), size(waveforms),1)
		...
		if ~isempty(vo)
			for y=1:logical_vars
				LFPavgPlot(duration, channel_plot, sort_spt{y}, psth_fig_handle(y), subhandle(y,:), sort_xy{y}, x_sel, y_sel, v_sel, o_sel, B(y,:),sfq)
				
			end
		else
			LFPavgPlot(duration, channel_plot, sort_spt, psth_fig_handle, subhandle, xy, x_sel, y_sel, v_sel, o_sel, [0 0], sfq)
